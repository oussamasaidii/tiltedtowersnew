...
export const MONGODB_URI = process.env.MONGODB_URI ?? 'mongodb://localhost:27017/Cluster0'
...
export async function register(username: string, email: string, password: string, game: string) {
    try {
        ...
        await userCollection.insertOne({ username, email, password: hashedPassword , role: "USER"});
    } catch (error) {
